# osint_functions.py - Все функции из твоего кода
import re
import aiohttp
import phonenumbers
        ...
        try:
            async with session.get(cdx_url) as response:
                if response.status != 200:
                    return {"Ошибка": f"Ошибка API: {response.status}"}
                
                data = await response.json()
                if len(data) <= 1:
                    return {"Результат": "Нет сохранений в архиве"}
                ...
                async with session.get(download_url) as json_response:
                    if json_response.status != 200:
                        return {"Ошибка": f"Ошибка скачивания: {json_response.status}"}
                    ...
                    if found_records:
                        ...
                        return {"Результат": "Номер не найден в базе"}
